...
import com.ikabi.actionbarsherlock.ActionBarSherlock.OnCreatePanelMenuListener;
import com.ikabi.actionbarsherlock.ActionBarSherlock.OnMenuItemSelectedListener;
import com.ikabi.actionbarsherlock.ActionBarSherlock.OnPreparePanelListener;
...
public abstract class Watson extends FragmentActivity implements OnCreatePanelMenuListener, OnPreparePanelListener, OnMenuItemSelectedListener {
    ...
    private ArrayList<android.support.v4.app.Fragment> mCreatedMenus;
    ...
    public boolean onCreatePanelMenu(int featureId, Menu menu) {
        ...
        if (featureId == Window.FEATURE_OPTIONS_PANEL) {
            ...
            boolean show = false;
            ArrayList<android.support.v4.app.Fragment> newMenus = null;
            if (mFragments.mAdded != null) {
                for (int i = 0; i < mFragments.mAdded.size(); i++) {
                    android.support.v4.app.Fragment f = mFragments.mAdded.get(i);
                    if (f != null && !f.mHidden && f.mHasMenu && f.mMenuVisible && f instanceof OnCreateOptionsMenuListener) {
                        show = true;
                        ((OnCreateOptionsMenuListener)f).onCreateOptionsMenu(menu, inflater);
                        if (newMenus == null) {
                            newMenus = new ArrayList<android.support.v4.app.Fragment>();
                        }
                        newMenus.add(f);
                    }
                }
